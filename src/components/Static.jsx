import React from 'react';

const Static = () => {
  return (
    <div  className=' bottom-0 fixed right-0 py-5 text-center'>
        <div className='py-2'><a href="">
            <a href="https://github.com/pedrocanddz">
                
            <img className='w-6 h-6 sm:w-8 sm:h-8' src="https://img.icons8.com/nolan/64/github.png" alt="github"/>
          
            </a>
          </a></div>
        <div className='sm:py-2'><a href="">
        <img className='w-6 h-6 sm:w-8 sm:h-8' src="https://img.icons8.com/color/48/linkedin.png" alt="linkedin"/>
          </a></div>
        <div className='sm:px-1 sm:py-2 sm:text-bold vertical-rl tracking-wider text-claro-neve'>pedrosousa@estudante.ufscar.br</div>
    </div>
  )
}

export default Static